// Aplicar fix de clientId nullable en tabla orders a todas las bases de datos
...
// Leer el archivo SQL
const sqlPath = path.join(__dirname, 'fix-order-clientid-nullable.sql');
const migrationSQL = fs.readFileSync(sqlPath, 'utf8');

// Obtener todas las URLs de bases de datos
const databases = {
...
async function applyMigration(dbName, connectionString) {
  ...
  try {
    ...
    // Verificar que el campo es nullable
    ...
    if (result.rows.length > 0 && result.rows[0].is_nullable === 'YES') {
      console.log(color('✅ Verificado: clientId es nullable', colors.green));
    } else {
...
async function main() {
  ...
  console.log(color('   FIX: CLIENTID NULLABLE EN TABLA ORDERS', colors.bright));
